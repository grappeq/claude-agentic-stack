---
description: Finalize & publish — sync with the base branch, confirm the Definition of Done, curate the commit, then push and open a PR. User-invoked; invoking it is your authorization to publish.
...
1. **Sync.** Run `/sync` to bring the branch up to date with its base so the PR opens conflict-free. If it returns `SYNC: BLOCKED`, STOP and report — do not publish a branch that will not cleanly integrate. `SYNC: CLEAN` or `SYNC: PASS` → continue (note `/sync` already re-ran the gate if it merged anything).

2. **Gate.** Confirm the work is shippable:
   - `/verify` is green (run it if you have not this turn — or if `/sync` reported `CLEAN` and you have not verified the current diff).
