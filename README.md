...
(Already have a `CLAUDE.md`? Merge the "Operating loop / Definition of Done / Orchestration contract" sections into yours.) Start Claude Code in the repo and run `/agents` to confirm the seven agents loaded, and `/` to see `/build`, `/prototype`, `/resume`, `/verify`, `/review`, `/sync`, `/ship`.
...
- **L3 — Human iteration.** You read the report (and screenshots) and send the next prompt, edit `.agentic/spec.md` to steer the next run, or `/resume` to continue an interrupted one. Always there.
...
- **Why two target files** — `task.md` and `spec.md` are the same pattern (durable target + live state) split by **authority**, not structure. The spec is a *user-confirmed contract*: drafted from your interview answers, approved at the spec checkpoint, then frozen — so a long unsupervised run converges on a target the agent can't quietly move; only the user edits it. The task file is the agent's own working restatement of a short, supervised change — continuously edited, never blessed, folding target and progress into one file because a single-milestone run doesn't need the split. One shared filename would let agent notes wear contract-grade authority (a misread task would be enshrined as if confirmed). The filename *is* the permission model: `spec.md` = the agent never edits it; `task.md` = the agent keeps it live.
