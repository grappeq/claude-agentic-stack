...
└── .claude/
    ...
    ├── agents/
    │   ├── product-designer.md        # kickoff Q&A + lean spec + mockup variants; gap review (read-only)
    │   ├── planner.md                 # decomposes a task → plan + tests + surface impact  (read-only)
    ...
    │   └── ux-reviewer.md             # critiques the rendered UI vs spec + design contract  (read-only)
    ├── skills/
    ...
    └── commands/
        ...
        ├── mockup.md     →  /mockup    # design checkpoint: variants → your pick → .agentic/design.md
        ├── resume.md     →  /resume    # re-enter an interrupted run from .agentic/ state
...
The runtime smoke writes screenshots — and `/mockup` writes mockups — under `.agentic/` — add **`.agentic/`** to your repo's `.gitignore` so that evidence (it can contain secrets or PII from the booted app) is never committed. As a backstop the stack also drops a `.agentic/.gitignore` of `*` on first run.

(Already have a `CLAUDE.md`? Merge the "Operating loop / Definition of Done / Orchestration contract" sections into yours.) Start Claude Code in the repo and run `/agents` to confirm the seven agents loaded, and `/` to see `/build`, `/prototype`, `/mockup`, `/resume`, `/verify`, `/review`, `/sync`, `/ship`.
...
- **`/prototype <vision>` — prototype.** Turning a broad vision into a working app: a **brief kickoff interview** (a few questions plus one design pick) pins down the need, then it runs **autonomously** — making the product and UX calls itself (recording assumptions) and **iterating across milestones** until the vision is met. Built for long, unattended runs.
...
/build <task>
   ...
   │  mockup†     ── /mockup when the plan reports a new surface → .agentic/design.md
   ...
   └─ report      ── auto-commit on green (work branch) ──► /ship to push / open a PR
                                          * ux-reviewer only when the diff touches UI
                                          † only for a new screen / endpoint / command / schema
```
...
/prototype <vision>
   ...
   │     → spec checkpoint (one confirm) → /mockup: pick a design for M1's key
   │     surfaces → .agentic/design.md frozen → gate closes, no more questions
   │          (headless: no Q&A or pick — defaults; /mockup still runs, design=auto)
   │  ┌─ per milestone, autonomous ───────────────────────────────────┐
   │  │  plan → mockup (auto, new surfaces) → implement → /verify      │
   │  │  REVIEW GATE: code ‖ security ‖ ux* → resolve until clean      │
   │  │     (stuck? circuit-breaker: 3 tries, then route around/stop)  │
   │  │  GAP REVIEW vs spec + design: ITERATE · ADVANCE · DONE · ASK   │
...
- **L3 — Human iteration.** You read the report (and screenshots) and send the next prompt, edit `.agentic/spec.md` or `.agentic/design.md` to steer the next run, or `/resume` to continue an interrupted one. Always there.

So `/build` is mostly L1; `/prototype` is L2 wrapping L1, then back to you (L3).

## Usage

- **Prototype from a vision:** `/prototype a habit tracker with streaks and weekly stats` — a few kickoff questions and one design pick, then it designs, builds, iterates across milestones, and screenshots it autonomously.
- **Precision change:** `/build add rate limiting to the login endpoint` — when the plan reports a new user-facing surface (a screen, endpoint, command, or schema) it stops once for a design pick; `design=auto` takes the recommended default, `design=skip` bypasses it.
- **Design checkpoint on its own:** `/mockup the settings screen` — 2–3 variants (web artboards, a CLI storyboard, API examples, a schema, or an architecture spine, by medium), your pick, frozen as a contract in `.agentic/design.md` that reviewers hold the build to.
...
- **Headless / CI:** `claude -p "/build add rate limiting to the login endpoint"` (the sandbox VM must be reachable via `ssh sandbox`; any design pick takes the recommended default)
...
- **Run artifacts** — a `/build` run writes its task, acceptance criteria, and live plan checklist to `.agentic/task.md`; a `/prototype` run writes its frozen vision to `.agentic/spec.md` and live progress to `.agentic/progress.md`; `/mockup` writes the approved design contract to `.agentic/design.md` with the mockups under `.agentic/design/` (all git-ignored). Read them to see where an autonomous run is, edit them between runs to redirect it, or `/resume` to continue it.
- **Design checkpoint** — edit `.claude/commands/mockup.md` to change the medium table (what a mockup *is* for web, CLI, API, library, schema, or architecture) or the re-draft cap (2); pass `design=ask|auto|skip` to `/build` or `/prototype` per run.
...
- **Generative + evaluative symmetry** — the stack pairs a *maker* and a *critic* for each concern: code (`planner` → `code-reviewer`/`security-reviewer`) product/UX (`product-designer` → `ux-reviewer`), and now design (`product-designer` DESIGN: PROPOSE → `ux-reviewer` design-fidelity check, or `e2e-tester` golden assertions for CLI/API surfaces), with `product-designer` also closing the loop by gap-checking the build against its own spec and the approved design.
...
- **Built for long autonomous runs** — `/prototype` front-loads a brief kickoff interview (spec + one design pick), then runs hands-off. The frozen vision (`.agentic/spec.md`) and a progress log (`.agentic/progress.md`) are durable, so a run survives context compaction; a circuit-breaker caps repeated failed fixes (3 tries) so a stuck check can't burn the session. `ux-reviewer` compares screenshots round-over-round to catch UI regressions.
- **Interruptible by design** — every run keeps a durable target on disk (`task.md` for `/build`, `spec.md` + `progress.md` for `/prototype`, `design.md` for either), so `/resume` can re-enter it from a *fresh* session: reload the state, reconcile it against git (the repo wins on code facts), re-verify, continue. Deliberately restarting with a clean context beats grinding on a degraded one — the useful half of the "Ralph loop" pattern, without the bash loop. Starting a new run surfaces any unfinished one instead of silently overwriting its state.
- **Design before code, by medium** — the design checkpoint (`/mockup`) exists because the cheapest moment to redirect a design is before it is built, and the loop otherwise shows you a design only after a milestone is built, verified, and reviewed. It is deliberately about the *surface* a user or caller experiences, not about pixels: web artboards, a CLI storyboard, API request/response examples, a schema, or an architecture spine — so it works for a backend service as well as a web app, and for text media the approved examples become the smoke assertions. `/build` only runs it when the plan reports a genuinely new surface (a change *to* an existing screen follows its design system), and `/prototype` asks once at kickoff and defaults later surfaces, so the "brief kickoff, then hands-off" promise holds. Web mockups are static HTML artboards rendered on the sandbox VM with Playwright; when the session has the built-in `design` skill's canvas (`/design`; a research preview — claude.ai login only, not API-key, Bedrock, or Vertex) it doubles as a nicer, hand-tweakable preview, but the stack never depends on it: the HTML + PNG under `.agentic/design/` are the evidence the read-only reviewers can actually open.
- **Why three target files** — `task.md`, `spec.md`, and `design.md` are the same pattern (durable target + live state) split by **authority**, not structure. The spec is a *user-confirmed contract*: drafted from your interview answers, approved at the spec checkpoint, then frozen — so a long unsupervised run converges on a target the agent can't quietly move; only the user edits it. The task file is the agent's own working restatement of a short, supervised change — continuously edited, never blessed, folding target and progress into one file because a single-milestone run doesn't need the split. One shared filename would let agent notes wear contract-grade authority (a misread task would be enshrined as if confirmed). The filename *is* the permission model: `spec.md` = the agent never edits it; `task.md` = the agent keeps it live. `design.md` is a second user-confirmed contract — a concrete artifact (mockup, transcript, examples) rather than a vision outline — kept separate from `spec.md` because it is approved at a different moment, per surface, and can be re-run with `/mockup` without reopening the vision; the same rule applies: only `/mockup` writes it, and each section records whether the user picked it or it was defaulted (assumption-grade, not contract-grade).
